...
const sendResponse = async (req, res, next, statusCode, data) => {
  ...
  if (statusCode >= 500) {
    if (process.env.NODE_ENV === 'staging' || process.env.NODE_ENV === 'production') {
      ...
      // );
      if (req.bugsnag) {
        req.bugsnag.user = req.bugsnag.user || {};
        req.bugsnag.user.bugSnagNotify = true;
      }
      if (req.user && req.bugsnag) {
        req.bugsnag.user.id = req.user._id;
        req.bugsnag.user.email = req.user.email;
      }
    }
    next(data);
  }
